...
export default class ToolBarView {
  ...
  constructor () {
    ...
    this.subscriptions.add(
      ...
      atom.config.observe('tool-bar-ide.iconSize', newValue => {
        this.updateSize(newValue);
      }),
      atom.config.onDidChange('tool-bar-ide.position', ({newValue, oldValue}) => {
        if (atom.config.get('tool-bar.visible')) {
          this.show();
        }
      }),
      atom.config.onDidChange('tool-bar-ide.visible', ({newValue, oldValue}) => {
        if (newValue) {
          this.show();
        } else {
          this.hide();
    ...
    if (supportFullWidth) {
      this.subscriptions.add(
        atom.config.onDidChange('tool-bar-ide.fullWidth', ({newValue, oldValue}) => {
          if (atom.config.get('tool-bar.visible')) {
            this.show();
          }
        })
    ...
    if (atom.config.get('tool-bar-ide.visible')) {
      this.show();
    }
  ...
  updatePosition (position) {
    ...
    const fullWidth = supportFullWidth && atom.config.get('tool-bar-ide.fullWidth');
  ...
  show () {
    this.hide();
    this.updatePosition(atom.config.get('tool-bar-ide.position'));
    this.updateSize(atom.config.get('tool-bar-ide.iconSize'));
  }

  toggle () {
    if (this.element.parentNode) {
      this.hide();
      atom.config.set('tool-bar-ide.visible', false);
    } else {
      this.show();
      atom.config.set('tool-bar-ide.visible', true);
    }
  }
}
