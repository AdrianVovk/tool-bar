...
export default class ToolBarView {
  ...
  constructor () {
    ...
    this.subscriptions.add(
      ...
      atom.config.onDidChange('tool-bar-ide.position', () => {
        if (atom.config.get('tool-bar.visible')) {
          this.show();
        }
      }),
      atom.config.onDidChange('tool-bar-ide.visible', ({newValue}) => {
        if (newValue) {
          this.show();
        } else {
          this.hide();
  ...
  updatePosition (position) {
    ...
    const fullWidth = atom.config.get('tool-bar-ide.fullWidth');
