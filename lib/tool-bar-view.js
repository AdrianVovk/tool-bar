...
export default class ToolBarView {
  ...
  constructor () {
    ...
    this.subscriptions.add(
      ...
      atom.commands.add('atom-workspace', 'toolbar:position-top', () => {
        this.updatePosition('Top');
        atom.config.set('toolbar-ide.position', 'Top');
      }),
      atom.commands.add('atom-workspace', 'toolbar:position-right', () => {
        this.updatePosition('Right');
        atom.config.set('toolbar-ide.position', 'Right');
      }),
      atom.commands.add('atom-workspace', 'toolbar:position-bottom', () => {
        this.updatePosition('Bottom');
        atom.config.set('toolbar-ide.position', 'Bottom');
      }),
      atom.commands.add('atom-workspace', 'toolbar:position-left', () => {
        this.updatePosition('Left');
        atom.config.set('toolbar-ide.position', 'Left');
      }),
      atom.config.observe('toolbar-ide.iconSize', newValue => {
        this.updateSize(newValue);
      }),
      atom.config.onDidChange('toolbar-ide.position', () => {
        if (atom.config.get('toolbar-ide.visible')) {
          this.show();
        }
      }),
      atom.config.onDidChange('toolbar-ide.visible', ({newValue}) => {
        if (newValue) {
          this.show();
        } else {
          this.hide();
      ...
      atom.config.onDidChange('toolbar-ide.fullWidth', () => {
        if (atom.config.get('toolbar-ide.visible')) {
          this.show();
        }
      })
  ...
  updatePosition (position) {
    ...
    const fullWidth = atom.config.get('toolbar-ide.fullWidth');
  ...
  show () {
    this.hide();
    this.updatePosition(atom.config.get('toolbar-ide.position'));
    this.updateSize(atom.config.get('toolbar-ide.iconSize'));
  }

  toggle () {
    atom.config.set('toolbar-ide.visible', !atom.config.get('toolbar-ide.visible'));
  }
}
